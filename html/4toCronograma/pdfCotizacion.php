<?php
	...
	$evento="Reporte de la cotizacion del 4to cornograma en pdf: ".$id_cotizacion;
	...
	$tlf=$_cod_tlf[$_beneficiario[$_cotizacion4tocronograma[$id_cotizacion]['id_empresa']]['id_cod_tlf']]['cod_tlf']."-".$_beneficiario[$_cotizacion4tocronograma[$id_cotizacion]['id_empresa']]['telefono'];
	if($_beneficiario[$_cotizacion4tocronograma[$id_cotizacion]['id_empresa']]['id_cod_tlf2']>0){
		$otrotlf=$_cod_tlf[$_beneficiario[$_cotizacion4tocronograma[$id_cotizacion]['id_empresa']]['id_cod_tlf2']]['cod_tlf']."-".$_beneficiario[$_cotizacion4tocronograma[$id_cotizacion]['id_empresa']]['telefono2'];
	}else{
		$otrotlf="";
	}
	$fecha=date("Y", strtotime($_cotizacion4tocronograma[$id_cotizacion]['fecha']));
	$pie=1;
	$encabezado="
		<div class='contenedor'>
			...
			<div align='right' style='font-size: 10px;'>
				<strong>PRESUPUESTO Nro &nbsp;&nbsp;VP-".$_cotizacion4tocronograma[$id_cotizacion]['cotizacion']." </strong>
			</div>
			<div>
				Fecha: ".$fec=date("d-m-Y", strtotime($_cotizacion4tocronograma[$id_cotizacion]['fecha']))."
			</div>
			<div class='clear'></div>
			<table class='header'>
				<tr>
					<td colspan='2' class='abajo'>NOMBRE (S) Y APELLIDO (S) Ó  RAZÓN SOCIAL:".$_beneficiario[$_cotizacion4tocronograma[$id_cotizacion]['id_empresa']]['nombre']."</td>
				</tr>
				<tr>
					<td class='abajo izquierda'>C.I. / RIF: ".$_beneficiario[$_cotizacion4tocronograma[$id_cotizacion]['id_empresa']]['rif']."</td>
					<td class='abajo '>DIRECCION: ".$_beneficiario[$_cotizacion4tocronograma[$id_cotizacion]['id_empresa']]['direccion']."</td>
				</tr>
				<tr>
					<td class='izquierda abajo'>TELÉFONO: ".$tlf."</td>
					<td class='abajo'>PERSONA DE CONTACTO: ".$_beneficiario[$_cotizacion4tocronograma[$id_cotizacion]['id_empresa']]['contacto']."</td>
				</tr>
				<tr>
		...
		$sql=$db->query("SELECT o.cantidad, p.producto, m.modelo, m.precio, o.precio_total FROM ordenes_4tocronograma o INNER JOIN producto p ON o.id_producto=p.id_producto INNER JOIN modelo m ON o.id_modelo=m.id_modelo WHERE id_cotizacion=$id_cotizacion;");
		if($db->rows($sql)>0){
			$contador=1;
		...
		if($_cotizacion4tocronograma[$id_cotizacion]['desc_porcentaje']!=0){
			$descuento="
				<tr>
					<td colspan='3' align='right' class='abajo izquierda'>
						<strong>CANTIDAD TOTAL:</strong>
					</td>
					<td class='abajo izquierda' align=center>".$_cotizacion4tocronograma[$id_cotizacion]['unidades_total']."</td>
					<td class='abajo'></td>
				</tr>
				<tr>
					...
					<td class='abajo' align='right'>".$_cotizacion4tocronograma[$id_cotizacion]['subtotal']."</td>
				</tr>
				<tr>
					<td colspan='3' align='right' class='abajo izquierda'>
						<strong>DESCUENTO:</strong>
					</td>
					<td class='abajo izquierda' align=center>".$_cotizacion4tocronograma[$id_cotizacion]['desc_porcentaje']."%</td>
					<td class='abajo' align='right'>".$_cotizacion4tocronograma[$id_cotizacion]['descuento']."</td>
				</tr>";
		...
		$footer="
					<tr>
						...
						<td align=right>".$_cotizacion4tocronograma[$id_cotizacion]['total']."</td>
					</tr>
				</table>
			</div>
		";
