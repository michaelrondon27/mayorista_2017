<?php
	...
	$evento="Factura de la cotizacion del 3er cornograma: ".$id_cotizacion;
	...
	$encabezado="
		<div class='contenedor'>
			...
			<table>
				...
				<tr>
					<td class='abajo izquierda'>Rif:".$_beneficiario[$_cotizacion3ercronograma[$id_cotizacion]['id_empresa']]['rif']."</td>
					<td class='abajo'>Razón Social: ".$_beneficiario[$_cotizacion3ercronograma[$id_cotizacion]['id_empresa']]['nombre']."</td>
				</tr>
				<tr>
					<td colspan='2' class='abajo '>Dirección: ".$_beneficiario[$_cotizacion3ercronograma[$id_cotizacion]['id_empresa']]['direccion']."</td>
				</tr>
				<tr>
					<td class='izquierda'>TELÉFONO: ".$_cod_tlf[$_beneficiario[$_cotizacion3ercronograma[$id_cotizacion]['id_empresa']]['id_cod_tlf']]['cod_tlf']."-".$_beneficiario[$_cotizacion3ercronograma[$id_cotizacion]['id_empresa']]['telefono']."</td>
	...
	$sql=$db->query("SELECT o.cantidad, p.producto, m.modelo, m.precio, o.precio_total, o.precio_num FROM ordenes_3ercronograma o INNER JOIN producto p ON o.id_producto=p.id_producto INNER JOIN modelo m ON o.id_modelo=m.id_modelo WHERE id_cotizacion=$id_cotizacion;");
	if($num=$db->rows($sql)>0){
		...
		while($data=$db->recorrer($sql)){
			...
			if($variable<=15){
				...
				$sub=$total;
				if($_cotizacion3ercronograma[$id_cotizacion]['desc_porcentaje']!=""){
					$porcentaje=$_cotizacion3ercronograma[$id_cotizacion]['desc_porcentaje']."%";
					$desc=$sub*$_cotizacion3ercronograma[$id_cotizacion]['desc_porcentaje']/100;
					$tot=$sub-$desc;
	...
	if($_cotizacion3ercronograma[$id_cotizacion]['desc_porcentaje']!=""){
		...
		if($id_cotizacion==232){
			$sub=$_cotizacion3ercronograma[$id_cotizacion]['total'];
		}
		$footer="
				<tr>
					<td colspan='4' align='right' class='izquierda abajo'>
