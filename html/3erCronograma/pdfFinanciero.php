<?php
	...
	$sql=$db->query("SELECT c.cotizacion, c.fecha, c.total, c.total_num, d.pago FROM cotizacion_3ercronograma c INNER JOIN despacho_3ercronograma d ON c.id_cotizacion=d.id_cotizacion WHERE c.despacho=1 AND c.fecha BETWEEN '$desde' AND '$hasta' ORDER BY c.fecha;");
	if($db->rows($sql)>0){
		$encabezado="
	...
	$reporte->SetTitle('Reporte Financiero del 3er cronograma desde '.$_POST['desde'].' hasta '.$_POST['hasta']);
	$reporte->Output('Reporte Financiero desde'.$_POST['desde'].' hasta '.$_POST['hasta'].'.pdf', 'I');
	$reporte->Output();
	}else{
		header("location: ?view=3erCronograma&mode=estadoFinanciero&error=2");
	}
?>