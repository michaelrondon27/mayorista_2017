<?php
	...
	$evento="Reporte de la orden de despacho de la cotizacion del 3er cronograma en pdf: ".$id_cotizacion;
	...
	$fecha=date("d-m-Y");
	$sql=$db->query("SELECT c.cotizacion, c.total, c.fecha, e.nombre, e.rif, e.direccion, e.telefono, e.contacto, ct.cod_tlf, c.descuento, c.desc_porcentaje, n.nro_orden, e.telefono2, e.correo, d.banco, d.deposito FROM cotizacion_3ercronograma c INNER JOIN empresa e ON c.id_empresa=e.id_empresa INNER JOIN cod_tlf ct ON e.id_cod_tlf=ct.id_cod_tlf INNER JOIN despacho_3ercronograma d ON c.id_cotizacion=d.id_cotizacion INNER JOIN nro_orden_3ercronograma n ON c.id_cotizacion=n.id_cotizacion WHERE c.id_cotizacion=$id_cotizacion LIMIT 1;");
	if($db->rows($sql)>0){
		...
		$sql1=$db->query("SELECT o.cantidad, p.producto, m.modelo FROM ordenes_3ercronograma o INNER JOIN producto p ON o.id_producto=p.id_producto INNER JOIN modelo m ON o.id_modelo=m.id_modelo WHERE o.id_cotizacion=$id_cotizacion;");
		if($db->rows($sql1)>0){
			$contador=1;
